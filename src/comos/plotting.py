...
from .tests import test_ambiguous,get_EDLogo_enrichment_scores

def plot_context_dependent_differences(
        motif, poi, sa, fwd_diff, rev_diff, 
        ymax=np.inf, pad=2, absolute=False, savepath=None):
    poi = int(poi)
    mlen = len(motif)

    motif_padded = ("N"*max(0, pad - poi) + 
                    motif + 
                    "N"*max(0,pad-(mlen-poi-1)))
    ...
    medians_fwd, Ns_med_fwd = sanamos.all_motif_medians(
        expanded_motifs, len(expanded_motifs[0]), 
        fwd_diff, rev_diff, sa, pad=0)
    ...
    for i in positions:
        fwd_diffs = fwd_diff[
            (ind_fwd + i)[(ind_fwd + i >= 0) & 
            (ind_fwd + i < len(fwd_diff))]]
        rev_diffs = rev_diff[
            (ind_rev - i)[(ind_rev - i >= 0) & 
            (ind_rev - i < len(rev_diff))]]
        diffs = np.concatenate([
            fwd_diffs[~np.isnan(fwd_diffs)], 
            rev_diffs[~np.isnan(rev_diffs)]])
    ...
    for i in range(len(expanded_motifs)):
        if absolute is False:
            ax.plot(
                ...
                medians_fwd[i], zorder=-50, alpha=0.5)
        else:
            ax.plot(
                range(1, len(positions)+1), 
                np.abs(medians_fwd[i]), zorder=-50, alpha=0.5)
...
def plot_diffs_with_complementary(
        motif, sa, fwd_diff, rev_diff, 
        ymax=np.inf, offset=3, savepath=None, absolute=True):
    mlen = len(motif)
    ...
    for c, (m, offset) in enumerate(zip(expl_motifs, offsets)):
        ...
        for i in positions:
            ...
            if absolute:
                diffs = np.abs(
                    np.concatenate([fwd_diffs[~np.isnan(fwd_diffs)], 
                    rev_diffs[~np.isnan(rev_diffs)]]))
            else:
                diffs = np.concatenate(
                    [fwd_diffs[~np.isnan(fwd_diffs)], 
        ...
        boxes.append(
            ax1.boxplot(
                data_fwd, positions=np.array(positions)+offset, widths=width, 
        ...
        ax2.boxplot(
            data_rev, positions=np.array(positions)+offset, widths=width, 
            flierprops={'marker': 'o', 'markersize': 3, 'alpha':0.5}, 
            patch_artist=True, 
            boxprops=dict(facecolor=f"C{c}"), 
            medianprops=dict(color="black",linewidth=1.5))
    ...
    ax1.legend(
        [boxes[i]["boxes"][0] for i in range(len(boxes))], 
        labels, loc='upper right', 
        prop={'family': 'monospace', 'size':10})
    ax1.set_xticks(positions)
    ax1.set_xticklabels([
        motif[i] if 0 <= i < mlen else "" 
        for i in positions])
    ax2.set_xticks(positions)
    ax2.set_xticklabels([
        motif[i].translate(comp_trans) if 0 <= i < mlen else "" 
        for i in positions])
    ax2.xaxis.tick_top()
    ...
    if absolute:
        ...
        max_ylim = (
            max(min(ax1.get_ylim()[0], ax2.get_ylim()[0]), -ymax), 
            min(max(ax1.get_ylim()[1], ax2.get_ylim()[1]), ymax))
...
def plot_motif_scores(
        results, mu, sigma, med, iqr, 
        thr=6., outlier_iqr=0., savepath=None):
    fig,ax = plt.subplots(figsize=(6,6))
    ax.scatter(results.N,results.val,s=1,alpha=0.25, color='black')
    X = np.unique(np.linspace(
            0, mu.shape[0]-1, min(mu.shape[0]-1, 1000)
        ).astype(int))
...
def letterAt(letter, x, y, yscale=1, ax=None):
    fontp = FontProperties(family="Arial", weight="bold")
    letters = { 
        "T" : TextPath((-0.305, 0), "T", size=1, prop=fontp),
        "G" : TextPath((-0.384, 0), "G", size=1, prop=fontp),
        "A" : TextPath((-0.350, 0), "A", size=1, prop=fontp),
        "C" : TextPath((-0.366, 0), "C", size=1, prop=fontp)}
    color_scheme = {
        'G': 'orange', 
        'A': 'red', 
        'C': 'blue', 
...
def plot_EDLogo(
        motif, poi, sa, mu, sigma, 
        fwd_metric, rev_metric, opt_dir, 
        min_N=10, stddev=3.0, n_pseudo=5, Nnull=500):
    poi = int(poi)
    fig,(ax_frac,ax) = plt.subplots(
        2, 1, sharex=True, 
    ...
    for ind in range(-1,len(motif)+1):
        df = get_EDLogo_enrichment_scores(
            motif, poi, ind, sa, mu, sigma, 
            fwd_metric, rev_metric, opt_dir, 
            min_N=min_N, stddev=stddev, 
            n_pseudo=n_pseudo, Nnull=Nnull)
        all_scores_positive.append(list(
            df.loc[df.r >= 0][['base', 'r']].itertuples(
                index=False, name=None)))
        all_scores_negative.append(list(
            df.loc[df.r < 0][['base', 'r']].itertuples(
                index=False, name=None)))
