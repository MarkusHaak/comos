...
from .helper import (
    ArgHelpFormatter, parse_rds_file, parse_bed_file, 
    parse_tombo_files, parse_largest_contig, parse_contigs)
from .metrics import (
    compute_expsumlogp, compute_diffs, compute_meanabsdiff, 
    compute_meanfrac)
from .motif import (
    reverse_complement, is_palindromic, combine_IUPAC, combine_motifs, 
    explode_motif, get_pois, motif_contains, motif_diff)
from .plotting import (
    plot_context_dependent_differences, plot_diffs_with_complementary, 
    plot_motif_scores, letterAt, plot_EDLogo)
from .tests import (
    test_ambiguous, get_EDLogo_enrichment_scores, test_replace, 
    test_replace_order, test_replace_total, test_exploded)
...
def parse_args():
    ...
    hyper_grp.add_argument(
        ...
        type=int,
        default=-0.1,
        help="minimum enrichment score in replace test."
    )
    hyper_grp.add_argument(
        '--outlier-iqr',
        type=float,
    ...
    if not os.path.exists(args.fasta):
        logging.getLogger('comos').error(
            f"Input file {args.fasta} does not exist.")
        exit(1)
    if not os.path.exists(args.mod):
        if not os.path.exists(os.path.dirname(args.mod)):
            logging.getLogger('comos').error(
                f"Input file {args.mod} does not exist.")
            exit(1)
        else:
            missing = []
            for suffix in [
                    '.valid_coverage.plus.wig', 
                    '.valid_coverage.minus.wig', 
                    '.dampened_fraction_modified_reads.plus.wig', 
                    '.dampened_fraction_modified_reads.minus.wig']:
                candidate_fns = [
                    fn for fn in os.listdir(os.path.dirname(args.mod)) 
                    if fn.startswith(os.path.basename(args.mod)) and 
                    fn.endswith(suffix)]
                if len(candidate_fns) != 1:
                    missing.append(suffix)
            if missing:
                logging.getLogger('comos').error(
                    f"Detected tombo input. Missing text-output files "
                    f"with suffixes {missing} for prefix {args.mod}.")
    ...
    if args.ambiguity_thr > args.selection_thr:
        logging.getLogger('comos').warning(
            f"Ambiguity threshold is set to selection threshold "
             "since it should not be larger.")
        args.ambiguity_thr = args.selection_thr
    return args

def get_seq_index_combinations(
        k_min, k_max, min_gap, max_gap, bases, blur):
        ...
        for comb in itertools.product(['A','C','G','T'], repeat=k):
            ...
            for gap in range(min_gap, max_gap+1):
                ...
                for gap_pos in gap_positions:
                    ...
                    if 'C' in bases:
                        valid = (valid or 
                            ('C' in comb[:gap_pos] and 
                             'G' in comb[gap_pos:]) or 
                            ('G' in comb[:gap_pos] and 
                             'C' in comb[gap_pos:]))
                    if 'A' in bases:
                        valid = (valid or 
                            ('A' in comb[:gap_pos] and 
                             'T' in comb[gap_pos:]) or 
                            ('T' in comb[:gap_pos] and 
                             'A' in comb[gap_pos:]))
                    if not valid:
                        continue
                    comb_ = (
                        comb[:gap_pos] + ["N"]*gap + comb[gap_pos:])
...
def normal_approx(d, outlier_iqr=0.):
    ...
    N = np.unique(np.concatenate(
        [np.geomspace(1,d.N.max(),min(d.N.max()-1,500)).astype(int), 
    ...
    return (np.interp(Ns, N, mu), np.interp(Ns, N, sigma), 
            np.interp(Ns, N, med), np.interp(Ns, N, iqrs))

def mutate_replace(
        motif, poi, sa, max_motif_len, mu, sigma, fwd_metric, 
        rev_metric, aggr_fct, opt_dir, min_N=30):
    poi = int(poi)
    likelihoods = []
    for i in range(len(motif)):
        ...
        df = get_EDLogo_enrichment_scores(
            motif, poi, i, sa, mu, sigma, fwd_metric, rev_metric, 
            opt_dir, min_N=min_N, stddev=3.0, n_pseudo=5, Nnull=500)
        bases = df.loc[df.r >= 0, 'base']
        for l in range(1,len(bases)+1):
            for comb in itertools.combinations(bases, l):
                 # TODO: optimize
                iupac = [b for b in IUPAC_TO_LIST 
                         if set(IUPAC_TO_LIST[b]) == set(comb)][0]
                likelihood = (
                    len(comb) * hmean(df.loc[df.base.isin(comb), 'r']) + 
                    (-df.loc[df.base.isin(
                        IUPAC_TO_LIST[IUPAC_NOT[iupac]]
                    ), 'r']).sum()) / df.r.abs().sum()
...
def resolve_nested_motif_graph(
        sG, d, sa, max_motif_len, mu, sigma, fwd_metric, rev_metric, 
        aggr_fct, opt_dir, thr, ambiguity_thr, bases="AC"):
    ...
    if draw:
        ...
        nx.draw_networkx(
            sG, nodelist=d.loc[d.index.isin(list(sG)) & ~d['pass']].index, 
            edgelist=[], node_color="red", pos=pos, ax=ax)
        if draw in sG:
            nx.draw_networkx(
                sG, nodelist=[draw], edgelist=sG.in_edges(draw), 
                node_color="green", edge_color="green", pos=pos, ax=ax)
    ...
    while True:
        ...
        for root_node in root_nodes:
            ...
            # check if node was previously flagged to be pruned 
            # but was not pruned yet due to previously unresolved 
            # incoming edges
            if d.loc[root_node, 'to_prune'] == True:
                pruned = prune_edges(sG, list(sG.out_edges(root_node)), d)
                d = d.drop(pruned)
                logging.getLogger('comos').debug(
                    f"pruned previously flagged root node "
                    f"{d.loc[root_node, 'motif']} and consequently "
                    f"pruned {len(pruned)} other nodes")
            ...
            pruned = prune_edges(
                sG, [edge for edge in sG.out_edges(root_node)], d)
            d = d.drop(pruned)
            logging.getLogger('comos').debug(
                f"kept root node {d.loc[root_node, 'motif']}, "
                f"pruned {len(pruned)} nodes")
            ...
            if draw:
                ...
                nx.draw_networkx(
                    sG, 
                    nodelist=d.loc[d.index.isin(list(sG)) & ~d['pass']].index, 
                    edgelist=[], node_color="red", pos=pos, ax=ax)
                if draw in sG:
                    nx.draw_networkx(
                        sG, nodelist=[draw], 
                        edgelist=sG.in_edges(draw), node_color="green", 
                        edge_color="green", pos=pos, ax=ax)
                plt.show(block=False)
    ...
    if len(d.loc[d.index.isin(all_sG_nodes)]) != len(list(sG)):
        d = d.drop(
            [n for n in all_sG_nodes if n in d.index and n not in list(sG)])
    return d

def recursively_remove_combined(
        n, v, sG, d, sa, max_motif_len, mu, sigma, fwd_metric, rev_metric, 
        aggr_fct, opt_dir, thr, ambiguity_thr, ambiguity_min_sites, 
        ambiguity_min_tests, replace_min_enrichment, bases="AC", 
        ambiguity_type='logo'):
    """
    Remove node n that is the result of a motif combination of v
    #but test if the difference n - v passes tests and shall remain.
    Also, recursively apply the same principle to nodes that were 
    combined from n
    """
    if n not in list(sG):
        return d
    ...
    for u in comb_nodes:
        if u in d.index:
            ...
            d = recursively_remove_combined(
                u, n, sG, d, sa, max_motif_len, mu, sigma, fwd_metric, 
                rev_metric, aggr_fct, opt_dir, thr, ambiguity_thr, 
                ambiguity_min_sites, ambiguity_min_tests, 
    ...
    ## TODO: in theory a nice idea, but I would need to properly integrate it 
    # into the Graph that is just being resolved
    #indices = []
    ...
    #       f"""Unhandled case: exploded motifs of the combined motif 
    #           {d.loc[u, 'motif']} are found at different indices in  
    #           {d.loc[v, 'motif']} that it was combined from"""
    #    idx = indices[0]
    #    diff = motif_diff(
    #       d.loc[n, 'motif'], 
    #       d.loc[v, 'motif'][idx:idx+len(d.loc[n, 'motif'])], 
    #       0, subtract_matching=True)
    #    aggregates, Ns = aggr_fct(
    #       [diff], len(diff), fwd_metric, rev_metric, sa, bases=bases)
    #    if opt_dir == "min":
    #        best_aggregate = np.nanmin(aggregates[0])
    #    else:
    #        best_aggregate = np.nanmax(aggregates[0])
    #    assert ~np.isnan(best_aggregate), 
    #       f"unhandled exception: found no motif aggregate for motif {diff}"
    #    if opt_dir == "min":
    ...
    #    passed = (
    #       test_ambiguous(diff, poi, sa, max_motif_len, mu, sigma, fwd_metric, 
    #           rev_metric, aggr_fct, opt_dir, ambiguity_thr, 
    #           min_N=ambiguity_min_sites, min_tests=ambiguity_min_tests, 
    #           bases=bases, ambiguity_type=ambiguity_type) and 
    #       test_exploded(diff, poi, sa, max_motif_len, mu, sigma, 
    ...
    #    if passed:
    #        new = pd.Series([
    #           diff, poi, aggregates[0][poi], Ns[0][poi], stddev, 
    #           norm().cdf(stddev), d.loc[n, 'bipartite'], None, False], 
    #           index=d.columns)
    #        if ~d.motif.eq(new.motif).any():
    #            logging.info(
    #               f"""Added motif {new.motif}:{poi} ({new.N}, 
    #                   {new.stddevs:.2f}) as difference 
    #                   {d.loc[n, 'motif']} - {d.loc[v, 'motif']}""")
    #            d.loc[d.index.max()+1] = new
    #    else:
    #        logging.info(
    #           f"""Motif {diff} as difference {d.loc[n, 'motif']} - 
    #               {d.loc[v, 'motif']} did not pass tests""")
    #else:
    #    logging.debug(
    #       f"""no exploded motif of the combined motif {d.loc[n, 'motif']} 
    #           is found in {d.loc[v, 'motif']} that it was combined from""")
    
    # TODO: If the above is not performed, it is probably best to just 
    # remove n and not prune all motifs that were combined to this one
    ...
    #   f"""pruned combined node {d.loc[n, 'motif']} and consequently pruned 
    #       {len(pruned)} other nodes""")
...
def final_motif_filter(
        motif, poi, sa, max_motif_len, mu, sigma, fwd_metric, 
        rev_metric, aggr_fct, opt_dir, min_N=30):
    passes  = (
        test_replace_order(
            motif, poi, sa, max_motif_len, mu, sigma, fwd_metric, rev_metric, 
            aggr_fct, opt_dir, min_N=min_N) and
        test_replace_total(
            motif, poi, sa, max_motif_len, mu, sigma, fwd_metric, rev_metric, 
            aggr_fct, opt_dir, 1., min_N=min_N))
    return passes

def resolve_combined_motif_graph(
        sG, d, sa, max_motif_len, mu, sigma, fwd_metric, rev_metric, aggr_fct, 
        opt_dir, thr, ambiguity_thr, ambiguity_min_sites, ambiguity_min_tests, 
        replace_min_enrichment, bases="AC", ambiguity_type='logo'):
    all_sG_nodes = list(sG)
    # resolve graph, starting with root nodes
    while True:
        root_nodes = [n for n,deg in sG.in_degree() if deg==0]
        for root_node in root_nodes:
            # check if node was previously flagged to be pruned but was 
            # not pruned yet due to previously unresolved incoming edges
            if d.loc[root_node, 'to_prune'] == True:
                pruned = prune_edges(sG, list(sG.out_edges(root_node)), d)
                d = d.drop(pruned)
                logging.getLogger('comos').info(
                    f"pruned previously flagged root node "
                    f"{d.loc[root_node, 'motif']} and consequently pruned "
                    f"{len(pruned)} other nodes")
                sG.remove_node(root_node)
                d = d.drop(root_node)
                break
            # check if node passes final filtering
            if not final_motif_filter(
                    d.loc[root_node, 'motif'], d.loc[root_node, 'poi'], sa, 
                    max_motif_len, mu, sigma, fwd_metric, rev_metric, aggr_fct, 
                    opt_dir, min_N=ambiguity_min_sites):
                logging.getLogger('comos').info(
                    f"removed root node {d.loc[root_node, 'motif']}, "
                    f"did not pass final filtering")
                sG.remove_node(root_node)
                d = d.drop(root_node)
                break
        else:
            ...
            for n in d.loc[d.index.isin(root_nodes) & (d.rc == d.index)].index:
                ...
                if list(sG.out_edges(n)):
                    ...
                    d = d.drop(pruned)
                    logging.getLogger('comos').info(
                        f"kept palindromic root node {d.loc[n, 'motif']} and "
                        f"consequently pruned {len(pruned)} other nodes")
                    break
            else:
                indices = d.index.copy() # to check if any nodes where pruned
                for n in (
                        d.loc[d.index.isin(root_nodes) & 
                        ~pd.isna(d.rc) & 
                        (d.rc != d.index)].index):
                    # prioritize motifs with RC over those without
                    # keep RC (which is not necessarily a root node) and handle 
                    # all motifs that it was combined to
                    # TODO: currently, for the RC, no final_motif_filter 
                    # is applied! Think about how to handle this
                    rc = d.loc[n, 'rc']
                    ...
                    for u in comb_nodes:
                        if u in d.index and u != n:
                            d = recursively_remove_combined(
                                u, rc, sG, d, sa, max_motif_len, mu, sigma, 
                    ...
                    if len(indices) != len(d.index):
                        logging.getLogger('comos').info(
                            f"kept root node {d.loc[n, 'motif']} (and its RC "
                            f"{d.loc[rc, 'motif']}) and consequently pruned "
                            f"{len(indices) - len(d.index)} other nodes")
                        break
                else:
                    for n in d.loc[
                            d.index.isin(root_nodes) & 
                            pd.isna(d.rc)].index:
                        # prune graph for nodes that were combined to this one 
                        # and its RC
                        if list(sG.out_edges(n)):
                            ...
                            d = d.drop(pruned)
                            logging.getLogger('comos').info(
                                f"kept root node {d.loc[n, 'motif']} and "
                                f"consequently pruned {len(pruned)} other "
                                f"nodes")
                            break
        if len(sG.edges) == 0 and ~np.any(d.to_prune):
            break
...
def score_motifs(
        motifs, max_motif_len, fwd_metric, rev_metric, sa, aggr_fct, opt_dir, 
        mu, sigma, bases="AC"):
    aggregates, Ns = aggr_fct(
        motifs, max_motif_len, fwd_metric, rev_metric, sa, bases=bases)
    with warnings.catch_warnings():
        warnings.filterwarnings('ignore', r'All-NaN slice encountered')
        if opt_dir == "min":
            best_aggregates = np.nanmin(aggregates, axis=1)
            poi = np.nanargmin(aggregates, axis=1)
        else:
            best_aggregates = np.nanmax(aggregates, axis=1)
    df = pd.DataFrame(best_aggregates, columns=['val'])
    df['motif'] = motifs
    df[['poi', 'N', 'Nclip', 'stddevs', 'p-value']] = np.nan
    sel = ~np.isnan(best_aggregates)
    if not np.any(sel):
        return df
    if opt_dir == "min":
    ...
    df.loc[sel, 'stddevs'] = (
        ...
        sigma[df.loc[sel, 'Nclip'].astype(int)])
    if opt_dir == "min":
        df.loc[sel, 'p-value'] = norm().cdf(df.loc[sel, 'stddevs'])
    else:
        df.loc[sel, 'p-value'] = 1. - norm().cdf(df.loc[sel, 'stddevs'])
    return df[['motif', 'poi', 'val', 'N', 'stddevs', 'p-value']]

def test_motifs(
        df, sa, max_motif_len, mu, sigma, fwd_metric, rev_metric, aggr_fct, 
        opt_dir, thr, ambiguity_thr, ambiguity_min_sites, ambiguity_min_tests, 
        replace_min_enrichment, bases=None, ambiguity_type=None):
    df.loc[:, 'ambiguity'] = df.apply(
        lambda row: test_ambiguous(
            row.motif, row.poi, sa, max_motif_len, mu, sigma, fwd_metric, 
            rev_metric, aggr_fct, opt_dir, ambiguity_thr, 
            min_N=ambiguity_min_sites, min_tests=ambiguity_min_tests,
            bases=bases, ambiguity_type=ambiguity_type), axis=1)
    if ambiguity_type == 'logo':
        df.loc[:, 'ambiguity'] &= df.apply(
            lambda row: test_replace(
                row.motif, row.poi, sa, max_motif_len, mu, sigma, fwd_metric, 
                rev_metric, aggr_fct, opt_dir, replace_min_enrichment, 
                min_N=ambiguity_min_sites), axis=1)
    df.loc[:, 'exploded'] = df.apply(
        lambda row: test_exploded(
            row.motif, row.poi, sa, max_motif_len, mu, sigma, fwd_metric, 
            rev_metric, aggr_fct, opt_dir, thr, bases=bases), axis=1)
    return df
...
def combine_and_test_motifs(
    ...
    scores_df = score_motifs(
        combined_motifs, max_motif_len, fwd_metric, rev_metric, sa, aggr_fct, 
        opt_dir, mu, sigma, bases=bases)
    if opt_dir == "min":
        scores_df = scores_df.loc[scores_df['stddevs'] <= thr]
        scores_df = scores_df.sort_values('stddevs', ascending=True)
    else:
        scores_df = scores_df.loc[scores_df['stddevs'] >= thr]
        scores_df = scores_df.sort_values('stddevs', ascending=False)
    if not scores_df.empty:
        best = test_motifs(
            scores_df.iloc[[0]], sa, max_motif_len, mu, sigma, fwd_metric, 
            rev_metric, aggr_fct, opt_dir, thr, ambiguity_thr, 
            ambiguity_min_sites, ambiguity_min_tests, replace_min_enrichment, 
            bases=bases, ambiguity_type=ambiguity_type)
        best = best.iloc[0]
        keep_combined = best['ambiguity'] and best['exploded']
    
    if keep_combined:
        drop_mi, drop_mj = True, True
    else:
        # check if any of the combined motifs is contained in one of the motifs 
        # that are being combined
        for motif in combined_motifs:
            if len(motif) <= len(d.loc[i].motif):
                idx, ident, diff_locs = motif_contains(
                    d.loc[i].motif, motif, d.loc[i].bipartite)
                if idx is not None:
                    if len(diff_locs) <= 1:
                        drop_mj = True
            if len(motif) <= len(d.loc[j].motif):
                idx, ident, diff_locs = motif_contains(
                    d.loc[j].motif, motif, d.loc[j].bipartite)
                if idx is not None:
                    if len(diff_locs) <= 1:
                        drop_mi = True
            if drop_mi and drop_mj:
                # is combined in both, take the shorter one or the 
                # better scoring one
                ...
                if len_i < len_j:
                    ...
                    drop_mj = False
                        ...
                        (opt_dir == "min" and 
                         d.loc[i].stddevs <= d.loc[j].stddevs) or 
                        (opt_dir == "max" and 
                         d.loc[i].stddevs >= d.loc[j].stddevs)):
                    drop_mi = False
                else:
                    drop_mj = False
    return best, keep_combined, drop_mi, drop_mj

def test_mutated_motif(
        mutated, d, i, sa, max_motif_len, mu, sigma, fwd_metric, rev_metric, 
        aggr_fct, opt_dir, thr, ambiguity_thr, ambiguity_min_sites, 
    ...
    if mutated != d.loc[i].motif and idx is not None:
        # strip here and not earlier in case mutated gets smaller, 
        # so that it still contains the original motif
        mutated = mutated.strip('N')
        
        scores_df = score_motifs(
            [mutated], max_motif_len, fwd_metric, rev_metric, sa, aggr_fct, 
            opt_dir, mu, sigma, bases=bases)
        if opt_dir == "min":
            scores_df = scores_df.loc[scores_df['stddevs'] <= thr]
            scores_df = scores_df.sort_values('stddevs', ascending=True)
        ...
        if not scores_df.empty:
            best = test_motifs(
                ...
                ambiguity_type=ambiguity_type)
            best = best.iloc[0]
            return best['ambiguity'] and best['exploded'], best
    return False, None

def reduce_motifs(
        d, sa, max_motif_len, mu, sigma, fwd_metric, rev_metric, aggr_fct, 
        opt_dir, thr, ambiguity_thr, ambiguity_min_sites, ambiguity_min_tests, 
        replace_min_enrichment, min_k, bases="AC", ambiguity_type='logo'):
    ...
    for i,row in d.loc[d['pass'] == False].iterrows():
        logging.getLogger('comos').debug(
            f"excluded {row.motif}:{row.poi} ({row.N}, {row.stddevs:.2f}) : "
            f"did not pass ambiguity filter")
    n_excluded = (d['pass'] == False).sum()
    if n_excluded:
        logging.info(
            f"Excluded {n_excluded:,} motifs because they did not pass "
            f"ambiguity testing.")
    #d = d.loc[d['pass']].drop(['pass'], axis=1)
    logging.getLogger('comos').info(
        f"{len(d.loc[d['pass']])} motifs after ambiguity testing")
    ...
    for i in range(0,len(d)):
        G.add_node(i)
        # TODO: check if longer edges needed, if not: 
        # & (d.slen - d.loc[i, 'slen'] <= 1)
        sel = (
            (d.index > i) & 
            (d.bipartite == d.loc[i, 'bipartite']) & 
            (0 <= d.slen - d.loc[i, 'slen']))
        contain_i = d.loc[sel].loc[
            d.loc[sel].motif.str.contains(d.loc[i].motif.replace('N','.'))]
    ...
    for c in nx.connected_components(G.to_undirected()):
        ...
        # drop entries that are not connected to any other by 
        # nesting and did not pass filtering
        if ~np.any(d.loc[list(sG), 'pass']):
            d = d.drop(list(sG))
        else:
            sGs.append(sG)
    logging.getLogger('comos').info(
        f"{n_sGs} subgraphs in nested motif network")
    logging.getLogger('comos').info(
        f"{len(sGs)} subgraphs with any valid motif")

    # fist reduce contiguous trees, then prune bipartite trees with contiguous 
    # motifs, then reduce bipartite trees
    # --> remove bipartite motifs that contain short contiguous motifs
    bipartite_sGs = [
        sG for sG in sGs if d.loc[list(sG)[0], "bipartite"]]
    non_bipartite_sGs = [
        sG for sG in sGs if not d.loc[list(sG)[0], "bipartite"]]
    d['to_prune'] = False
    for sG in non_bipartite_sGs:
        d = resolve_nested_motif_graph(
            sG, d, sa, max_motif_len, mu, sigma, fwd_metric, rev_metric, 
        ...
        for i,_ in d.loc[~d.bipartite].iterrows():
            ...
            while changed:
                ...
                for j,_ in d.loc[list(sG)].iterrows():
                    ...
                    idx, ident, diff = motif_contains(
                        d.loc[j].motif, d.loc[i].motif, bipartite=True)
                    if idx is not None:
                        pruned = prune_edges(sG, list(sG.out_edges(j)), d)
                        sG.remove_node(j)
                        logging.getLogger('comos').debug(
                            f"Motif {d.loc[i, 'motif']} found in biparite "
                            f"motif {d.loc[j, 'motif']}, "
                            f"pruned {len(pruned)+1} nodes")
                        d = d.drop(j)
                        d = d.drop(pruned)
                        changed = True
                        break
    ...
    logging.info(
        f"{len(d)} canonical motifs remaining after removing nested motifs:")
    print(d)
    ...
    while changed == True:
        ...
        for i in d.loc[d.bipartite & pd.isna(d.rc)].index:
            for j in d.loc[d.bipartite].index:
                idx, ident, diff = motif_contains(
                    reverse_complement(d.loc[i].motif), d.loc[j].motif, 
                    bipartite=True)
                if idx is not None and len(diff) == 1:
                    ...
                    if not scores_df.empty:
                        ...
                        new['rc'] = i
                        logging.info(
                            f"Motif {d.loc[j].motif} is contained in RC of "
                            f"motif {d.loc[i].motif} which misses a RC --> "
                            f"keep {new.motif}:{new.poi} ({new.N}, "
                            f"{new.stddevs:.2f}) over {d.loc[i].motif}:"
                            f"{d.loc[i].poi}({d.loc[i].N}, "
                            f"{d.loc[i].stddevs:.2f})")
                        d = d.drop(i)
                        if ~d.motif.eq(new.motif).any():
                            d.loc[d.index.max() + 1] = new
                        changed = True
                        break
                idx, ident, diff = motif_contains(
                    reverse_complement(d.loc[j].motif), 
                    d.loc[i].motif, bipartite=True)
                if idx is not None and len(diff) == 1 and not d.loc[j].rc:
                    ...
                    if not scores_df.empty:
                        ...
                        new['rc'] = i
                        logging.info(
                            f"Motif {d.loc[i].motif} is contained in RC of "
                            f"motif {d.loc[j].motif}, both missing a RC --> "
                            f"keep {new.motif}:{new.poi} ({new.N}, "
                            f"{new.stddevs:.2f}) over {d.loc[j].motif}:"
                            f"{d.loc[j].poi}({d.loc[j].N}, "
                            f"{d.loc[j].stddevs:.2f})")
                        d = d.drop(j)
                        if ~d.motif.eq(new.motif).any():
                            d.loc[d.index.max() + 1] = new
                        changed = True
    ...
    logging.info(
        f"removing {sel.sum()} bipartite canonical motifs due to missing RC")
    d = d.drop(d.loc[sel].index)
    
    logging.info(
        f"{len(d)} canonical motifs remaining after rule-based filtering:")
    print(d)
    ...
    while changed == True:
        ...
        for i in d.index:
            ...
            if d.loc[i].motif not in tested:
                ...
                mutated, probs = mutate_replace(
                    d.loc[i].motif, d.loc[i].poi, sa, len(d.loc[i].motif), mu, 
                    sigma, fwd_metric, rev_metric, aggr_fct, opt_dir, 
                    min_N=ambiguity_min_sites)
                mutated = "".join([
                    b if p >= 0.9 else o 
                    for o,b,p in zip(d.loc[i].motif, mutated, probs)])
                passed, new = test_mutated_motif(
                    mutated, d, i, sa, max_motif_len, mu, sigma, fwd_metric, 
                    rev_metric, aggr_fct, opt_dir, thr, ambiguity_thr, 
                    ambiguity_min_sites, ambiguity_min_tests, 
                    replace_min_enrichment, min_k, bases=bases, 
                    ambiguity_type=ambiguity_type)
                if passed:
                    # add to df
                    new['bipartite'] = d.loc[i].bipartite
                    if ~d.motif.eq(new.motif).any():
                        new_index = d.index.max() + 1
                        d.loc[new_index] = new
                        logging.getLogger('comos').info(
                            f"add new mutated motif {new.motif} based on "
                            f"{d.loc[i].motif}")
                    else:
                    ...
                    if d.loc[i].rc and d.loc[i].rc != i:
                        ...
                        passed, new = test_mutated_motif(
                            mutated_rc, d, rc, sa, max_motif_len, mu, sigma, 
                            fwd_metric, rev_metric, aggr_fct, opt_dir, thr, 
                            ambiguity_thr, ambiguity_min_sites, 
                            ambiguity_min_tests, replace_min_enrichment, 
                            min_k, bases=bases, ambiguity_type=ambiguity_type)
                        if passed:
                            # add to df
                            new['bipartite'] = d.loc[rc].bipartite
                            if ~d.motif.eq(new.motif).any():
                                new_index = d.index.max() + 1
                                d.loc[new_index] = new
                                logging.getLogger('comos').info(
                                    f"add new mutated motif {new.motif} based "
                                    f"on {d.loc[rc].motif}, the RC of "
                                    f"{d.loc[i].motif}")
                            else:
                                new_index = d.loc[
                                    d.motif.eq(new.motif)].index[0]
                            if new_index != rc:
                                G.add_edge(new_index, rc)
                        else:
                            logging.getLogger('comos').info(
                                f"mutated motif {mutated_rc}, the RC of "
                                f"mutated motif {mutated}, failed to pass")
            ...
            for j in d.loc[i+1:].index:
                ...
                new, keep_combined, drop_mi, drop_mj = combine_and_test_motifs(
                    d, i, j, sa, max_motif_len, mu, sigma, fwd_metric, 
                    rev_metric, aggr_fct, opt_dir, thr, ambiguity_thr, 
                    ambiguity_min_sites, ambiguity_min_tests, 
                    replace_min_enrichment, min_k, bases=bases, 
                    ambiguity_type=ambiguity_type)
                if keep_combined:
                    new['bipartite'] = d.loc[i].bipartite
                    #new = pd.Series(new+[None], index=d.columns)
                    if ~d.motif.eq(new.motif).any():
                        new_index = d.index.max() + 1
                        d.loc[new_index] = new
                    else:
                    ...
                    logging.getLogger('comos').error(
                        f"unhandled case: both original motifs "
                        f"({d.loc[i].motif} and {d.loc[j].motif}) "
                        f"shall be dropped")
                    exit(1)
    ...
    try:
        ...
        logging.getLogger('comos').error(
            f"unhandled case: cycle in combined graph")
        exit(1)
    except:
        pass
    ...
    if draw:
        ...
        nx.draw_networkx(
            G, nodelist=d.loc[d.index.isin(list(G)) & pd.isna(d['rc'])].index, 
            edgelist=[], node_color="red", pos=pos, ax=ax)
        nx.draw_networkx(
            G, nodelist=d.loc[d.index.isin(list(G)) & (d.index == d.rc)].index, 
            edgelist=[], node_color="green", pos=pos, ax=ax)
        nx.draw_networkx(
            G, nodelist=[n for n,deg in G.in_degree() if deg==0], 
            edgelist=[], node_color="yellow", node_size=100, pos=pos, ax=ax)
        plt.show(block=False)
        print(d)
        breakpoint()

    d['to_prune'] = False
    d = resolve_combined_motif_graph(
        G, d, sa, max_motif_len, mu, sigma, fwd_metric, rev_metric, aggr_fct, 
        opt_dir, thr, ambiguity_thr, ambiguity_min_sites, ambiguity_min_tests, 
        replace_min_enrichment, bases=bases, ambiguity_type=ambiguity_type)
    logging.info(
        f"{len(d)} combined motifs remaining after resolving the "
        f"combined motif graph:")
    print(d)
        ...
        for i in range(0,len(d)):
            for j in range(i+1,len(d)):
                ...
                if d.iloc[i].motif == reverse_complement(d.iloc[j].motif):
                    if ((opt_dir == "min" and 
                            d.iloc[i].stddevs <= d.iloc[j].stddevs) or 
                        (opt_dir == "max" and 
                            d.iloc[i].stddevs >= d.iloc[j].stddevs)):
                        logging.getLogger('comos').debug(
                            f"flagged {d.iloc[j].motif}:{d.iloc[j].poi} "
                            f"({d.iloc[j].N}, {d.iloc[j].stddevs:.2f}) as "
                            f"reverse complement of {d.iloc[i].motif}:"
                            f"{d.iloc[i].poi} ({d.iloc[i].N}, "
                            f"{d.iloc[i].stddevs:.2f})")
                        d.iloc[j, d.columns.get_loc('representative')] = \
                            d.iloc[i].motif
                        changed = True
                        break
                    else:
                        logging.getLogger('comos').debug(
                            f"flagged {d.iloc[i].motif}:{d.iloc[i].poi} "
                            f"({d.iloc[i].N}, {d.iloc[i].stddevs:.2f}) as "
                            f"reverse complement of {d.iloc[j].motif}:"
                            f"{d.iloc[j].poi} ({d.iloc[j].N}, "
                            f"{d.iloc[j].stddevs:.2f})")
                        d.iloc[i, d.columns.get_loc('representative')] = \
                            d.iloc[j].motif
                        changed = True
                        break
            if changed:
...
def find_methylated_motifs(
    ...
    cache_fp = os.path.join(
        args.cache, 
        f"{os.path.abspath(args.fasta).replace('/','_')}_{contig_id}_"
        f"{args.bases}_{args.metric}_{args.aggregate}_c{args.min_cov}_"
        f"w{args.window}_wv{args.min_window_values}_"
        f"b{args.subtract_background}_k{args.min_k}-{args.max_k}_"
        f"g{args.min_g}-{args.max_g}.pkl")
    if os.path.exists(cache_fp) and args.cache:
        ...
        aggr_metric, aggr_metric_counts = aggr_fct(
            all_canon_motifs, args.max_k + args.max_g, fwd_metric, rev_metric, 
            sa, bases=args.bases)
        tstop = time.time()
        logging.getLogger('comos').info(
            f"Computed canonical motif scores in {tstop - tstart:.2f} seconds")
        if args.cache:
    ...
    results['N'] = aggr_metric_counts[mask][
        range(results['poi'].shape[0]), results['poi']]
    # change order of columns to match that in function reduce_motifs
    results = results[['poi', 'val', 'N']] 

    tstart = time.time()
    mu, sigma, med, iqr = normal_approx(results, outlier_iqr=args.outlier_iqr)
    tstop = time.time()
    logging.getLogger('comos').info(
        f"Performed normal approximation in {tstop - tstart:.2f} seconds")
    ...
    if args.test_motifs:
        res = score_motifs(
            args.test_motifs, np.max([len(m) for m in args.test_motifs]), 
            fwd_metric, rev_metric, sa, aggr_fct, opt_dir, mu, sigma, 
            bases=args.bases)
        res = res.set_index('motif')

        for m,row in res.iterrows():
            print(f"\nAmbiguous-Test results for motif {m}:{int(row.poi)}")
            passed = test_ambiguous(
                m, row.poi, sa, len(m), mu, sigma, fwd_metric, rev_metric, 
                aggr_fct, opt_dir, ambiguity_thr, 
                min_N=args.ambiguity_min_sites, debug=True, 
                min_tests=args.ambiguity_min_tests, bases=args.bases, 
            ...
            if args.ambiguity_type == 'logo':
                ...
                passed = test_replace(
                    m, row.poi, sa, len(m), mu, sigma, fwd_metric, rev_metric, 
                    aggr_fct, opt_dir, args.replace_min_enrichment, 
                    min_N=args.ambiguity_min_sites)
                print("Passed:", passed, '\n')

            print(f"\nExploded-Test results for motif {m}:{int(row.poi)}")
            passed = test_exploded(
                m, row.poi, sa, len(m), mu, sigma, fwd_metric, rev_metric, 
                aggr_fct, opt_dir, selection_thr, debug=True, bases=args.bases)
            print("Passed:", passed, '\n')

            plot_EDLogo(
                m, row.poi, sa, mu, sigma, fwd_metric, rev_metric, opt_dir, 
                min_N=args.ambiguity_min_sites, stddev=3.0, n_pseudo=5, 
                Nnull=500)

            print(mutate_replace(
                m, row.poi, sa, len(m), mu, sigma, fwd_metric, rev_metric, 
                aggr_fct, opt_dir, min_N=args.ambiguity_min_sites))
        
        if args.plot and args.mod.lower().endswith(".rds") and 0:
            fwd_diff, rev_diff = compute_diffs(df, seq)
            for m,row in res.iterrows():
                plot_context_dependent_differences(
                    m, row.poi, sa, fwd_diff, rev_diff, 
                    savepath=os.path.join(
                        res_dir, f"{m}_{int(row.poi)}_median.png"))
                plot_diffs_with_complementary(
                    m, sa, fwd_diff, rev_diff, 
                    savepath=os.path.join(
                        res_dir, 
                        f"{m}_{int(row.poi)}.png"), 
                    ymax=10., absolute=True)
    ...
    if args.plot:
        plot_motif_scores(
            results, mu, sigma, med, iqr, thr=selection_thr, 
            outlier_iqr=args.outlier_iqr, 
            savepath=os.path.join(res_dir, f"scores_scatterplot.png"))
    ...
    tstop = time.time()
    logging.getLogger('comos').info(
        f"Performed motif reduction in {tstop - tstart:.2f} seconds")
    motifs_found['palindromic'] = motifs_found['motif'].apply(is_palindromic)
    motifs_found = motifs_found[[
        'motif', 'poi', 'representative', 'bipartite', 'palindromic', 'val', 
        'N', 'stddevs', 'p-value']]
    logging.getLogger('comos').info(
        f"Reduced to {len(motifs_found)} motifs in "
        f"{len(motifs_found.representative.unique())} MTase groups:")
    print(motifs_found.set_index(['representative', 'motif']).sort_values(
        by=['bipartite', 'palindromic', 'representative', 'stddevs'], 
        ascending=[True, True, True, opt_dir=='min']))
    
    if args.plot and args.mod.lower().endswith('.rds') and 0:
        fwd_diff, rev_diff = compute_diffs(df, seq)
        for r,row in motifs_found.loc[
                motifs_found.motif == motifs_found.representative].iterrows():
            plot_diffs_with_complementary(
                row.motif, sa, fwd_diff, rev_diff, absolute=True, 
                savepath=os.path.join(res_dir, f"{row.motif}_{row.poi}.png"))
...
def main():
    ...
    sufficient_length = [
        contig_id for contig_id, seq in contigs.items() 
        if len(seq) >= args.min_seq_len]
    logging.getLogger('comos').info(
        f"Dataset contains {len(contigs)} contig"
        f"{'s' if len(contigs) > 1 else ''}")
    if not sufficient_length:
        logging.getLogger('comos').error(
            f"No sequence in {args.fasta} is of sufficient length of >= "
            f"{args.min_seq_len:,} bp, please adjust argument --min-seq-len")
        exit(1)
    if len(sufficient_length) < len(contigs):
        logging.getLogger('comos').warning(
            f"Only {len(sufficient_length)} contigs >= {args.min_seq_len:,} "
            f"bp out of {len(contigs)} are analyzed")
    
    cache_fp = os.path.join(
        args.cache, 
        f"{os.path.abspath(args.mod).replace('/','_')}.pkl")
    if os.path.exists(cache_fp) and args.cache:
        tstart = time.time()
        df_all = pd.read_pickle(cache_fp)
        tstop = time.time()
        logging.getLogger('comos').info(
            f"Parsed cached mod data in {tstop - tstart:.2f} seconds")
    else:
        tstart = time.time()
        ...
        tstop = time.time()
        logging.getLogger('comos').info(
            f"Parsed mod file in {tstop - tstart:.2f} seconds")
        if args.cache:
            df_all.to_pickle(cache_fp)
    
    canon_motifs = get_seq_index_combinations(
        args.min_k, args.max_k, args.min_g, args.max_g, list(args.bases), 0)
    all_canon_motifs = list(canon_motifs.keys())
    logging.getLogger('comos').info(
        f"Analyzing {len(canon_motifs):,} canonical motifs and "
    ...
    if args.metagenome:
        for contig_id in sufficient_length:
            ...
            if args.cache:
                cache_fp = os.path.join(
                    args.cache, 
                    f"{os.path.abspath(args.fasta).replace('/', '_')}-"
                    f"{contig_id}.idx")
            sa = sanamos.get_suffix_array(contig_id, seq, cache_fp=cache_fp)
            df = df_all.loc[df_all.contig == contig_id]
            if args.metric == "expsumlogp":
                fwd_metric, rev_metric = compute_expsumlogp(
                    df, seq, args.min_cov, args.window, args.min_window_values, 
                    args.subtract_background)
            elif args.metric == "meanabsdiff":
                fwd_metric, rev_metric = compute_meanabsdiff(
                    df, seq, args.min_cov, args.window, args.min_window_values, 
                    args.subtract_background)
            elif args.metric == "meanfrac":
                fwd_metric, rev_metric = compute_meanfrac(
                    df, seq, args.min_cov, args.window, args.min_window_values, 
                    args.subtract_background)
            ...
            logging.getLogger('comos').info(
                f"Analyzing contig {contig_id} of length "
                f"{len(contigs[contig_id]):,}")
            find_methylated_motifs(
                all_canon_motifs, fwd_metric, rev_metric, sa, res_dir, 
                contig_id, args)
    else:
        ...
        res_dir = args.out
        find_methylated_motifs(
            all_canon_motifs, fwd_metrics, rev_metrics, SAs, res_dir, "all", 
            args)
